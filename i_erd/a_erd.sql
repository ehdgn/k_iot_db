...
# 1:N - 한 엔티티가 여러 개와 관계 (교수 - 강의, 학생 - 수강, 게시글 - 댓글)
...
# >> 관계: 1명의 학생은 여러 개의 수강 내역을 가질 수 있음 (1:N관계)

# 3) 예시 2
# 주문과 주문 상세 내역
# 주문 테이블) 부모 - 주문 ID PK, 주문 일시... 등
# 주문 상세 내역 테이블) 자식 - 주문 ID PK, 주뭄ㄴ 상품 ... 등
...
#		- 회원 없이도 자식 테이블이 존재 가능한 구조로 설계 가능 (1:N 관계 - 점선관계)

# cf) N:M 관계 처리 방법
# : 관계형 데이터베이스는 직접적인 N:M 관계를 하용하지 않음
# - 중간 테이블을 사용하여 분해
# EX) 학생 - 과목
...
create table course_eonrllment (
	student_id bigint,
    course_id bigint,
    enrollemt_date date,
